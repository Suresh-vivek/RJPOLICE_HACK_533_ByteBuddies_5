...
function checkHelplineNumber(number, callback) {
  // Check if the number is present in the helplineData array
  for (const entry of helplineData) {
    if (entry.helplines.includes(number)) {
      callback("Number present for " + entry.institution);
    } else {
      callback("Number not present in the dataset.");
    }
  }
}

chrome.commands.onCommand.addListener(function (command) {
  if (command === "checkNumber") {
      ...
      chrome.tabs.sendMessage(
        ...
        function (response) {
          ...
          if (/^\d+$/.test(selectedText)) {
            checkHelplineNumber(selectedText, function (message) {
              alert(message);
            });
          }
        }
